  ...
  const handleDrag = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    return () => {
      container.removeEventListener(
        "mousedown",
        startDragging as EventListener
      );
      container.removeEventListener("mouseup", stopDragging);
      container.removeEventListener("mouseleave", stopDragging);
      container.removeEventListener(
        "mousemove",
        whileDragging as EventListener
      );
      container.removeEventListener(
        "touchstart",
        startDragging as EventListener
      ...
      <div className="w-full gap-3 flex flex-col h-full ">
        <div className="w-full h-fit sm:h-[205px] bg-[#d9d9d9]/20 border-[0.8px] border-black/20 rounded-[11px] p-3 gap-2 flex flex-col">
            ...
            <div className="size-[min(40vw,90px)] relative">
              ...
              {data.verified && (
                <img
                  src={verifiedIcon}
                  alt="verified icon"
                  className="absolute -top-0 -right-1 size-[26px] z-10 "
                />
              )}
            </div>
          ...
          <div className="w-full flex flex-col gap-2">
            <a
              href={`/residential/${data.licenseNo}/${UrlGenerator(
                data.title
              )}`}
        ...
        <div className="w-full h-full bg-[#d9d9d9]/20 border-[0.8px] border-black/20 rounded-[11px] p-3 gap-3 flex flex-col">
          <div className="w-full flex flex-col sm:flex-row justify-between gap-4 ">
            ...
            <div className=" px-2 w-fit min-w-[48px] h-[15px] rounded-l-full rounded-r-full bg-[#2d2d2d] text-white flex flex-row items-center justify-center gap-1">
              ...
              <p className="text-[8px]">{data.category}</p>
            </div>
          </div>
          ...
          <div className="flex flex-row gap-1 items-center">
            ...
            <p className="text-[11px] text-[#2d2d2d]">
              {data.location.length > 45
                ? data.location.slice(0, 45) + "..."
                : data.location}
            </p>
          </div>
          <div className="w-full items-center text-[10px] ">
            ...
            >
              {" "}
              Read more{" "}
              <span className="text-[#4BAE4F] text-[11px]">{">"}</span>
            </a>
          </div>
          ...
          <div className="w-full flex flex-row gap-2">
            <div className="w-[48px] h-[25px] bg-white rounded-l-full rounded-r-full border-[1px] border-[#f2f2f2] items-center justify-center flex flex-row gap-2">
              ...
              <p className="text-[13px] text-[#757575]">{data.bedrooms}</p>
            </div>

            <div className="w-[48px] h-[25px] bg-white rounded-l-full rounded-r-full border-[1px] border-[#f2f2f2] items-center justify-center flex flex-row gap-2">
              <img src={bathIcon} alt="Bed icon" className="size-[15px]" />
              <p className="text-[13px] text-[#757575]">{data.bathrooms}</p>
            </div>

            <div className="w-[48px] h-[25px] bg-white rounded-l-full rounded-r-full border-[1px] border-[#f2f2f2] items-center justify-center flex flex-row gap-2">
              <img src={garageIcon} alt="Bed icon" className="size-[15px]" />
              <p className="text-[13px] text-[#757575]">{data.garageSpaces}</p>
            </div>
            ...
            <div className="w-[78px] h-[25px] bg-white rounded-l-full rounded-r-full border-[1px] border-[#f2f2f2] items-center justify-center flex flex-row gap-2">
              ...
              <p className="text-[13px] text-[#757575]">{data.landSize}</p>
            </div>
          </div>
          <div className="w-full flex flex-row items-center justify-center py-1">
            <div className="w-[80%] h-[1px] bg-black/10" />
          ...
          {data.propertyImages.slice(0, 6).map((image, index) =>
            index === 5 ? (
              <div
                ...
                key={index}
                className="w-full h-[105px] cursor-pointer rounded-[11px] overflow-hidden relative"
              >
                <img
                  src={image}
                  alt="property"
              ...
              <div
                ...
                key={index}
                className="w-full h-[105px] cursor-pointer rounded-[11px] overflow-hidden"
              >
                <img
                  src={image}
                  alt="property"
              ...
              <div className="w-[150px] flex items-center justify-center">
                ...
                >
                  <img
                    src={arrowLeftWhiteIcon}
                    alt="left arrow"
                    className="w-[10px] h-[16px] group-hover:-translate-x-1 ease-in-out transition-all duration-300"
                  />
                </div>
              </div>
            ...
            >
              ...
              {data.propertyImages.map((image, index) => (
                <div
                  ...
                  className={`${
                    index === currentViewImage
                      ? "scale-125 border-2 border-white z-10"
                      : "scale-100 border border-gray-300"
                  } w-[100px] h-[60px] md:w-[150px] md:h-[90px] rounded-[8px] transition-transform duration-300 cursor-pointer flex-shrink-0`}
                >
                  <img
                    src={image}
                    alt="property"
